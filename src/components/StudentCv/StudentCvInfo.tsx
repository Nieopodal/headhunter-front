  ...
  return <>
      ...
      <BodyOfSection>
        <StarsSection
          ...
          amount={studentData.student_course_completion}
        />
        <StarsSection
        ...
        <StarsSection
          ...
          amount={studentData.student_project_degree}
        />
        <StarsSection
          title="Ocena pracy w zespole Scrum"
          amount={studentData.student_team_project_degree}
        />
      </BodyOfSection>
      ...
      <BodyOfSection additionalClasses="grid grid-cols-2 md:grid-cols-3 lg:flex">
        <ExpectationCategory
          ...
          body={studentData.student_expected_type_work}
        />
        <ExpectationCategory
          title="Docelowe miasto, gdzie chce pracować kandydat"
          body={
            studentData.student_expected_salary.length > 0
              ? studentData.student_target_work_city
              : "Nie podano"
          }
        />
        <ExpectationCategory
          title="Oczekiwany typ kontraktu"
          body={studentData.student_expected_contract_type}
        />
        <ExpectationCategory
          title="Oczekiwane wynagrodzenie miesięczne netto"
          body={
            Number(studentData.student_expected_salary) > 0
              ? numberWithSpaces(studentData.student_expected_salary)
              : ""
          }
        />
        <ExpectationCategory
          ...
          body={
            studentData.student_can_take_apprenticeship === 1 ? "TAK" : "NIE"
          }
        />
        <ExpectationCategory
          title="Komercyjne doświadczenie w programowaniu"
          body={`${
            studentData.student_months_of_commercial_exp
          } ${monthDeclension(studentData.student_months_of_commercial_exp)}`}
        />
      </BodyOfSection>
      ...
      <BodyOfSection color="#0B8BD4">
        <div className="block">
          {studentData.student_portfolio_urls &&
            arrayFromStringHandler(studentData.student_portfolio_urls).map(
              (el, i) => <OneLink url={el} key={i} />
            )}
        </div>
      </BodyOfSection>
      ...
      <BodyOfSection color="#0B8BD4">
        <div className="block">
          {studentData.student_scrum_project_urls &&
            arrayFromStringHandler(studentData.student_scrum_project_urls).map(
              (el, i) => <OneLink url={el} key={i} />
            )}
        </div>
      </BodyOfSection>
      ...
      <BodyOfSection color="#0B8BD4">
        <div className="block">
          {studentData.student_project_urls &&
            arrayFromStringHandler(studentData.student_project_urls).map(
              (el, i) => <OneLink url={el} key={i} />
            )}
        </div>
      </BodyOfSection>
