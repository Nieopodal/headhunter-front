...
export const FilteringModal = () => {
  const { setIsFiltering, currentFilters, setCurrentFilters } =
    useContext(HrFilteringContext);
  ...
  const onFilterSearchSubmit = async (filters: HrFilteringCriteria) => {
    const res = await fetchApi(
      user,
      `${apiUrl}/hr/set-filter/`,
      "POST",
      "Wystąpił błąd przy próbie zastosowania filtrów",
      filters,
      true,
      "application/json"
    );
  ...
  const handleFilterOff = async () => {
    ...
    await fetchApi(
      user,
      `${apiUrl}/hr/remove-filter/`,
      "GET",
      "Wystąpił błąd przy anulowaniu filtrów"
    );
  ...
  return (
    <div className="flex flex-col items-start justify-between gap-10 max-w-[550px] h-fit h-fit">
      ...
      <FormProvider {...methods}>
        <form
          onSubmit={methods.handleSubmit(onFilterSearchSubmit)}
