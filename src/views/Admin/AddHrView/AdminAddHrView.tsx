...
export const AdminAddHrView = () => {
  ...
  return (
    ...
    >
      <form
        className="flex flex-col gap-8 mb-10 justify-items-start"
      ...
      >
        <FormProvider {...methods}>
          <div className="flex flex-col">
            ...
            {errors?.fullName && (
              <ResponseParagraph text={errors.fullName.message as string} />
            )}
          </div>
          <div className="flex flex-col">
            <Input type="email" name="email" placeholder="E-mail" />
            {errors?.email && (
              <ResponseParagraph text={errors.email.message as string} />
            )}
          </div>
          <div className="flex flex-col">
            <Input type="text" name="company" placeholder="Nazwa firmy" />
            {errors?.company && (
              <ResponseParagraph text={errors.company.message as string} />
            )}
          </div>
          <div className="flex flex-col">
            <Input
            ...
            {errors?.maxReservedStudents && (
              <ResponseParagraph
                text={errors.maxReservedStudents.message as string}
              />
            )}
          </div>
          ...
          {(apiData as ApiData) && !apiError && (
            <ResponseParagraph
              isSuccess
              text={`Pomyślnie dodano nową osobę HR o ID: ${
                (apiData as ApiData).id
              }`}
            />
          )}
          <button className="btn-md w-full btn-primary normal-case font-normal text-base">
            Dodaj osobę HR
          </button>
        </FormProvider>
      </form>
